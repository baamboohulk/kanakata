...
export function generateNBestJa2Vi(katakana: string, preset: Preset, nBest: number) {
  ...
  if (!tokens || !Array.isArray(tokens) || tokens.length === 0) {
    return { tokens: [], lattice: [], paths: [] as any[] };
  }
  ...
  const lattice: TokenAlt[] = tokens.map((t, i) => {
    ...
    if (i >= tokens.length - 2) {
      ...
      if (inDb.length) alts = inDb;
      else alts = alts.slice(0, 2); // giảm rác
    }
  ...
  for (let i = 0; i < lattice.length; i++) {
    ...
    for (const b of beam) {
      for (let k = 0; k < alts.length; k++) {
        ...
        // Phạt fallback romaji (không dấu)
  ...
  const paths = beam
    ...
    .map((b) => {
      const words = b.s.map((w, idxWord) => {
        ...
        // ƯU TIÊN "tên thật" cho 2 từ cuối
        ...
        if (idxWord === 1) {
          const prefer =
            ex.find((x) => x.toLowerCase() === "thị") ||
            ex.find((x) => x.toLowerCase() === "văn");
  ...
  // DEBUG (tạm thời)
  // console.log("[ja2vi] tokens=", tokens, "lattice=", lattice.length, "paths=", paths.length);
